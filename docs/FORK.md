...
```text
origin   https://github.com/SanHsien/agent-cortex.git
...
**PR、push、release 一律指向 `SanHsien/agent-cortex`。** 對 `hamanpaul/paulsha-cortex` 開 PR、
push 或發 release，需要維護者在當次對話明確同意回貢；「fork 一份」「建開發環境」「比照其他 repo」
都不是同意。
...
```powershell
gh repo set-default SanHsien/agent-cortex   # 每個 clone 先跑一次
gh repo set-default --view                    # 必須回 SanHsien/agent-cortex
gh pr create --repo SanHsien/agent-cortex --base main --head feature/<slug>
```
...
所以這裡仍走 branch → PR → CI → merge，只是 base 一定是 `SanHsien/agent-cortex`。
