...
> **Fork status**
> `SanHsien/agent-cortex` 是 [`hamanpaul/paulsha-cortex`](https://github.com/hamanpaul/paulsha-cortex) 的 Windows 原生開發發行版。它保留 Linux/systemd 相容性，但以原生 Windows 11 + PowerShell + Python 作為第一級開發與 CI 路徑。此 fork 用於研究、開發與驗證 agent engineering governance；目前不宣稱 production-ready。
...
```powershell
pipx install git+https://github.com/SanHsien/agent-cortex.git
cortex --help
cortex --version
```
...
```powershell
pipx install "git+https://github.com/SanHsien/agent-cortex.git@<commit-sha>"
```
